...
let is_some_digit a = match a with None -> false | Some a -> Char.is_digit a

let take_in_direction g dir =
  Grid.take_while_in_direction g dir ~f:(fun c ->
      if Char.is_digit c then Some c else None)
  |> Iter.to_str

let move_and_take g dir =
  Option.(
    Grid.move g dir >>= fun l ->
    let s = take_in_direction l dir in
    if String.(s = "") then None else Some s)
...
let take_in_row g =
  if Grid.extract g |> Char.is_digit then
    ...
    let toRight =
      move_and_take g (1, 0)
      |> Option.map ~f:(fun a -> int_of_string a |> singleton)
    in
    let toLeft =
      move_and_take g (-1, 0)
      |> Option.map ~f:(fun a -> String.rev a |> int_of_string |> singleton)
    in
...
let numbersAround (g : char gridComonad) =
  ...
  let top_nums =
    Option.(Grid.move g (0, 1) >>= fun o -> Some (take_in_row o))
  in
  let bottom_nums =
    Option.(Grid.move g (0, -1) >>= fun o -> Some (take_in_row o))
  in
...
let get_loc_sum g =
  let loc = Grid.extract g in
  if is_part loc then
    let nums = numbersAround g in
    List.fold ~f:( + ) ~init:0 nums
  else 0
...
let partB (i : input) : string =
  Grid.extend_from_grid i get_loc_sumb |> Grid.sum |> string_of_int

let tests =
  "tests"
  >::: [
         golden_test "day3" parser partA "4361" ~printer:show_input;
